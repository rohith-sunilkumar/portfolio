...
export const getWorkProjectById = (id) => {
  return workProjects.find(project => project.id === id);
};

export const getPlaygroundItems = () => {
  return playgroundItems;
};

export const getPlaygroundItemById = (id) => {
  return playgroundItems.find(item => item.id === id);
};

export const createPlaygroundItem = (item) => {
  const newItem = {
    ...item,
    id: item.id || item.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
  };
  playgroundItems.push(newItem);
  return newItem;
};

export const updatePlaygroundItem = (id, updates) => {
  const index = playgroundItems.findIndex(item => item.id === id);
  if (index === -1) return null;
  playgroundItems[index] = { ...playgroundItems[index], ...updates, id };
  return playgroundItems[index];
};

export const deletePlaygroundItem = (id) => {
  const index = playgroundItems.findIndex(item => item.id === id);
  if (index === -1) return false;
  playgroundItems.splice(index, 1);
  return true;
};
